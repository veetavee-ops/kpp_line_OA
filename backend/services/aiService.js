...
async function summarizeAllChatsForDate(allMessages) {
  try {
    ...
    console.log(`📝 Summarizing with Gemini 2.5: ${allMessages.length} msgs | ${uniqueDayKeys.length} day(s) | ${allGroupKeys.size} group(s)`);
...
${chatSummaryText}`;
    ...
    console.log(`✅ Summary generated by Groq (${uniqueDayKeys.length} day(s))`);
    ...
    if (error.response?.status === 401) {
      throw new Error('GROQ_API_KEY ไม่ถูกต้อง ตรวจสอบใน .env');
    }

    if (error.response?.status === 429) {
      throw new Error('ใช้งาน Groq API เกิน rate limit กรุณารอสักครู่แล้วลองใหม่');
    }
